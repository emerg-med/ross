...
if ($arr_request[action]=='Update'){
	...
	#put into quality control all patients whose triage is changed
	if ($arr_request[TriageCategory]<>$arr_request[oldtriagecat]){
		...
		$str_sql = "INSERT INTO Quality SET UR='$arr_request[UR]',  QualTimeStamp='".time()."', QualityText='$quality', FirstName= '$arr_request[FirstName]', Surname='$arr_request[Surname]' ";
		$result = mysql_db_query($dbname, $str_sql, $id_link);
		if (! $result) { affy_error_exit('SQL Select Execution has failed.');} 	
